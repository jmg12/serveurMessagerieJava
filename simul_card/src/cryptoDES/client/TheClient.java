...
// TO COMPLETE
    byte[]apdu=new byte[challenge.length+6];
			byte [] header = {CLA_TEST, typeINS, P1_EMPTY, P2_EMPTY, (byte)challenge.length};
      System.arraycopy(header, 0, apdu, 0, header.length);
			System.arraycopy(challenge, 0, apdu, header.length, challenge.length);
      apdu[apdu.length-1] = apdu[4];

			CommandAPDU cmd = new CommandAPDU( apdu );
			ResponseAPDU resp = this.sendAPDU( cmd, true );
			// Enovyer 8 octet ou multiple de 8
			//ajouter du padding ( valeur des octets ajouté = au nombre d'octet ajouté)
    ...
    private void foo() {
	    ...
	    byte[] unciphered;
	    long seed=0;
			// Générateur aleo basé le point de départ seed
