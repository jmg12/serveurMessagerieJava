...
public class TheClient {
	...
	void enterWritePIN() {
		// PIN 11
		System.out.println("S'il vous plaît entrer le code PIN : ");
		...
		System.arraycopy(bpin, 0, apdu, 5, pin.length());
	...
	void readNameFromCard() {
		...
		byte[] hapdu = {CLA,READNAMEFROMCARD,P1,P2,0};
		CommandAPDU cmd = new CommandAPDU( hapdu );
    ...
    String msg = "";
		...
		System.out.println("Le nom de l'etudiant contenu dans la carte est : ");
	...
	void writeNameToCard() {
		...
		System.out.println("S'il vous plaît entrer le nom de l'etudiant : ");
		String nomEtudiant = readKeyboard();
		byte [] bnomEtudiant = nomEtudiant.getBytes();
		byte [] apdu = new byte[5 + nomEtudiant.length()];
		byte [] hapdu = {CLA, WRITENAMETOCARD, P1, P2, (byte)nomEtudiant.length()};

		System.arraycopy(hapdu, 0, apdu, 0, 5);
		System.arraycopy(bnomEtudiant, 0, apdu, 5, nomEtudiant.length());
